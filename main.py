...
# NB: I had to manually change some 
if size == 'Small':
    ...
    RSI_clean = pd.read_csv(
        'Collection Code Output/AV- FTSE 100 RSI.csv',
        index_col='EOMONTH'
        )
    ME_clean = pd.read_csv(
        'Collection Code Output/AV- FTSE 100 ME.csv',
        index_col='EOMONTH'
        )
    Profit_clean = pd.read_csv(
    ...
    Beta_clean = pd.read_csv(
        'Collection Code Output/FTSE 100 Beta.csv',
        index_col='EOMONTH'
        )
...
Coefficient_timeline = pd.DataFrame(columns=['RSI','VAL','GP','Beta','LAG'], index=Executable_Dates['EOMONTH'])
Dataset = pd.DataFrame()
Purchases = pd.DataFrame(columns=['1st Choice','2nd Choice','3rd Choice'], index=Executable_Dates['EOMONTH'])
PL = pd.DataFrame(columns=['1st Choice','2nd Choice','3rd Choice'], index=Executable_Dates['EOMONTH'])
...
for a in Executable_Dates.index:
    ...
    for company_name in Companies['Company name']:
        Company_df = Header.company_dataset(company_name, chosen_dates, ME_clean, SE_clean, RSI_clean, Profit_clean, Beta_clean, hold_for)
    ...
    PL.at[end_date,'1st Choice'] = Payoff.at[end_date, Purchases.at[end_date,'1st Choice']]
    PL.at[end_date,'2nd Choice'] = Payoff.at[end_date, Purchases.at[end_date,'2nd Choice']]
    PL.at[end_date,'3rd Choice'] = Payoff.at[end_date, Purchases.at[end_date,'3rd Choice']]
...
Purchases.to_csv('Results/'+ size+ 'Cap holding for '+ str(hold_for)+ ' mths- purchases.csv')
comparison.to_csv('Results/'+ size+ 'Cap holding for '+ str(hold_for)+ ' mths- performance.csv')
Coefficient_timeline.to_csv('Results/'+ size+ 'Cap holding for '+ str(hold_for)+ ' mths- Coefficients.csv')
